...
against_currecny = 'USD'

start = dt.datetime(2019,1,1)
end = dt.datetime.now()

data = web.DataReader(f'{crypto_currency}-{against_currecny}','yahoo',start,end)
...
test_data = web.DataReader(f'{crypto_currency}-{against_currecny}','yahoo',test_start, test_end)
...
plt.title( crypto_currency+' Price Prediciton')
plt.xlabel('Time')
plt.ylabel('Price')
